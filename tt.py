...
F = 0
mutation_rate = 0.01
tt = [ [ [ 'n/a' for k in range(Classes) ] for j in range(Periods) ] for i in range(Days) ] 
...
classsubfreq = [[ lec_per_sub_per_clas for i in range(len(subs)) ] for j in range(Classes) ]

def sublockerIni(k):
    if k ==2:
        #sets the frequency of occurance of a subject in a perticular day to zero initially
        return 0
    else:
...
for i in range(Days):
    for j in range(Periods):
        for k in range(Classes):
            ...
            while classsubfreq[j][random_sub]==0:
                random_sub = random.randint(0,len(subs)-1)
            sublocker[random_sub][i][0][j] +=1
            sublocker[random_sub][i][1][k] +=1
            sublocker[random_sub][i][2]+=1
            tt[i][j][k]=random_sub
            classsubfreq[j][random_sub]-=1
